...
| Nivel | Qué debes definir | Preguntas guía | Ejemplo (Registro de usuarios) |
| --- | --- | --- | --- |
| **Épico** | Meta SMART o Resultado esperado | ¿Qué cambio de negocio buscas? ¿Cómo lo medirás? | "Reducir abandono en registro en 20%" |
| **Historia** | User Story alineada a INVEST | ¿Quién obtiene el valor? ¿Cuál es la acción? ¿Qué beneficio recibe? | "Como visitante, quiero crear una cuenta con mi email para guardar mi progreso" |
| **AC** | Criterios `Dado/Cuando/Entonces` | ¿Cómo sabrás que funciona? ¿Qué errores debes mostrar? | Ingresar datos válidos crea la cuenta, email inválido muestra error, duplicados se bloquean |
| **Tareas técnicas** | Pasos atómicos | ¿Qué debes construir en frontend, backend, pruebas y despliegue? | Formulario, endpoint `POST /users`, hash de contraseña, seed de datos |
| **Spike** | Investigación acotada | ¿Qué no conoces aún? ¿Cuánto tiempo invertirás? | Evaluar proveedor de emails transaccionales (4h) |
| **Pruebas** | Casos manuales o automatizados | ¿Qué validaciones cubrirás? ¿Dónde quedarán documentadas? | Test e2e de registro feliz, prueba de error, verificación de email |
...
1.  **Elige tu formato**: Puede ser Trello, Notion, hoja de cálculo o un archivo Markdown en tu repositorio.
2.  **Puebla tu Backlog**: Lista cada historia (una tarjeta o un encabezado) con sus puntos, tareas y estado.
3.  **Prioriza con MoSCoW**: Usa etiquetas o texto para indicar Must/Should/Could. Si trabajas en Markdown, añade un campo `Prioridad`.
4.  **Planifica tu Primer Sprint**: Mueve o marca 2-3 historias "Must" que quepan en tu capacidad calculada.

Si eliges Markdown, inicia con esta estructura mínima y añade una subsección por historia para documentar sus criterios de aceptación y tareas técnicas:

```md
## Sprint 1 – Objetivo
- Capacidad comprometida: 7 pts
- Historias: US-01 (Must, 3 pts), US-02 (Must, 2 pts), US-05 (Should, 2 pts)
...
### US-01 – Registro con email (3 pts)
- **AC**: 
  - Dado que ingreso un email válido, cuando envío el formulario, entonces recibo confirmación.
  - Dado un email duplicado, cuando intento registrarme, entonces veo un error.
- **Tareas técnicas**:
  - Crear formulario de registro.
  - Implementar endpoint `POST /users`.
  - Escribir prueba e2e del caso feliz.

### US-02 – ...
```

Lo importante es que toda historia tenga un lugar donde se describa lo acordado (AC) y lo que ejecutarás (tareas técnicas), aunque sea en texto plano.
</Aside>

<LinkCard
  title="Plantilla Notion – Roadmap Personal"
  href="https://www.notion.so/templates/product-roadmap"
  description="Duplica esta plantilla si prefieres Notion; si no, usa la estructura Markdown anterior como base."
/>
